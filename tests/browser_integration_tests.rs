...
mod browser_integration {
    ...
    async fn test_navigate_and_snapshot() {
        let tool = BrowserTool::new(TrustLevel::Full).expect("agent-browser must be installed");
    ...
    async fn test_write_guard_blocks_in_autonomous_mode() {
        let tool =
            BrowserTool::new(TrustLevel::Autonomous).expect("agent-browser must be installed");

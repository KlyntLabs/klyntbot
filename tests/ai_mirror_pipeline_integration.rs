...
async fn skill_routed_event_persists_routing_snapshot_via_ai_pipeline() {
    ...
    let started =
        cognitive::mirror::MirrorEngine::start(mirror_repo.clone(), None, None, None, None, None);
...
async fn finance_events_produce_drift_snapshot() {
    ...
    let source = Arc::new(cognitive::mirror::sources::FinanceSpendingDriftSource::new(
        mirror_repo.clone(),
    ));
    let cancel = tokio_util::sync::CancellationToken::new();
    let runner = ai_core::MirrorSubscriberRunner::new(source.clone(), cancel.clone());
    ...
    let snap = mirror_repo
        .get_latest_finance_drift_snapshot()
        .await
        .unwrap()
        .unwrap();

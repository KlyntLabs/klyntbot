...
impl CalendarHandler for MockCalendarHandler {
    async fn sync_calendar(&self) -> ::common::Result<Value> {
        Ok(serde_json::json!({"status": "mock"}))
    }

    async fn list_events(&self, _limit: usize) -> ::common::Result<Value> {
    ...
    async fn get_status(&self) -> ::common::Result<Value> {
        Ok(serde_json::json!({"status": "ok"}))
    }

    async fn get_event(&self, uid: &str) -> ::common::Result<Option<CalendarEvent>> {
        let events = self.events.lock().unwrap();
        Ok(events.iter().find(|e| e.uid == uid).cloned())
    }

    async fn get_events_for_reconciliation(&self) -> ::common::Result<Vec<CalendarEvent>> {
        Ok(self.events.lock().unwrap().clone())
    }

    async fn push_single_task(&self, _task_id: &str) -> ::common::Result<Value> {
        Ok(serde_json::json!({"status": "pushed"}))
    }

    async fn remove_single_event(&self, _calendar_event_uid: &str) -> ::common::Result<Value> {
        Ok(serde_json::json!({"status": "removed"}))
    }

    async fn pull_all_events(&self) -> ::common::Result<Value> {
        Ok(serde_json::json!({"status": "success", "events_pulled": 0}))
    }

    async fn push_all_tasks(&self) -> ::common::Result<Value> {

...
async fn spawn_capped_then_resume_to_completion() {
    ...
    let provider: providers::DynProvider =
        providers::testing::SingleResponseProvider::dyn_arc("done");
    ...
    let r1 = rt
        .spawn(agent::subagent_runtime::SpawnParams {
    ...
    let r2 = rt
        .resume(agent::subagent_runtime::ResumeParams {
...
async fn resume_on_running_returns_error() {
    ...
    sqlx::query(
        "INSERT INTO sessions (key, mode) VALUES ('parent-B', 'assistant'), ('sub-B', 'subagent')",
    )
    .execute(pool.inner())
    .await
    .unwrap();
    repos
        .subagent_instances
        .insert(&storage::repos::NewSubagentInstance {

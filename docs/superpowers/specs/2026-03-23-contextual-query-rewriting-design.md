...
1. **In Step 5.5**, query `TaskRepo::list_focused().await?.into_iter().next()` — returns the
   first task in a focus slot, if any. (`get_focused()` does not exist; `list_focused()` is
   generated by the `focus_impl!` macro and returns all focused tasks.)

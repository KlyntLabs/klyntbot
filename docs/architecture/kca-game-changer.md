...
|---|---|---|---|---|---|---|---|---|
| comprehensive-001 | 2026-05-02 13:00–18:52 | 10 (full) | 624 (partial) | GRADER 429 | **26.3%** (164/624) | — | — | **Wave 1-4 bundle measured.** Flags: `KCA_PHASE_3=1 KCA_PHASE_4=1 KCA_AUDD=1 KCA_DISABLE_COMPRESSION=1`. Stack: FTS-as-candidate (Wave 1), entity_aliases populate+expand (Wave 2), AUDD `LlmConflictResolver` (Wave 3), `speaker` column on facts + bench compression off (Wave 4), Phase 3 graph consolidation (`facts_processed=500` confirmed). Per-cat: cat1=**38.5% (+10.5pp ✅)**, cat2=23.2% (+1.8), cat3=39.0% (-5), cat4=**20.9% (-12pp ❌)**. Net **-2.5pp** vs baseline — cat-1 single-hop strongly improved (the original target), but cat-4 temporal regressed in same pattern as failed P1. Hypothesis: `LlmConflictResolver` over-supersedes temporally-distinct facts (UPDATE classification on `Alice→event=X` vs prior `Alice→event=Y`). Run terminated by OpenAI quota exhaustion at n=624 (~93% complete) — partial but statistically meaningful (CI ±~3.6pp). Pre-existing graph_link JSON-parse drift logged throughout (handler degrades to empty output, non-blocking). **Wave bundle does not yet ship**; cat-4 fix required first. |
| p1-confirm-001 | 2026-05-01 17:00–01:24 | 10 (full) | 569 | OK | **23.3%** (133/569) | — | — | **Phase 1 hypothesis FALSIFIED at n=500.** Speaker-prefix prompt addendum (`KCA_PHASE_1=1`) net **-5.5pp** vs baseline. Per-cat: cat1=27.9% (-0.1), cat2=22.3% (+0.9), cat3=40.6% (-3.5), cat4=**19.3% (-49.1pp catastrophic)**. Dev set conv-1+2 had shown +13.2pp peak — confirmed structural mismatch with full corpus. 100% of C-grades are zero-retrieval (read-side bottleneck, not extraction). p95=68.7s (over 60s gate). All three revert triggers fired. **Reverted prompt addendum**, kept defensive pronoun deny-list. |

...
function reducer(s: State, a: Action): State {
  switch (a.type) {
    ...
    case "moveSelection": {
      const next = Math.max(0, Math.min(s.results.length - 1, s.selectedIndex + a.delta));
...
export function LauncherStoreProvider({ children }: { children: ReactNode }) {
  ...
  const api = useMemo<LauncherStoreApi>(
    () => ({
      ...
      setResults: (results) => dispatch({ type: "patch", patch: { results, isSearching: false } }),
      setSelectedIndex: (selectedIndex) => dispatch({ type: "patch", patch: { selectedIndex } }),
      setIsSearching: (isSearching) => dispatch({ type: "patch", patch: { isSearching } }),

      moveSelection: (delta) => dispatch({ type: "moveSelection", delta }),
      pushHistory: (query) => dispatch({ type: "pushHistory", query }),
      navigateHistory: (direction) => dispatch({ type: "navigateHistory", direction }),
      setDetailItem: (detailItem) => dispatch({ type: "patch", patch: { detailItem } }),
      setActionMenuOpen: (actionMenuOpen) => dispatch({ type: "patch", patch: { actionMenuOpen } }),
      setArgModeItem: (argModeItem) => dispatch({ type: "patch", patch: { argModeItem } }),
...
export function useLauncher<T>(selector: (s: State & LauncherStoreApi) => T): T {
  const state = useContext(StateCtx);
  const api = useContext(ApiCtx);
  if (!state || !api) throw new Error("LauncherStoreProvider missing");
  const merged = useMemo(() => ({ ...state, ...api }) as State & LauncherStoreApi, [state, api]);

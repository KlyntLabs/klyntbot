    ...
    <div className="relative flex items-center gap-2 border-b px-4 py-1.5 bg-muted/20">
        ...
        {selectedAgent ? (
          <>
            ...
            <span className="font-medium">
              {selectedAgent.description || selectedAgent.agent_id.slice(0, 8)}
            </span>
            <span
              className={`h-1.5 w-1.5 rounded-full ${TYPE_DOTS[selectedAgent.subagent_type] ?? "bg-indigo-500"}`}
            />
      ...
      <div className="flex items-center gap-1 ml-1">
        ...
        {agents.slice(0, 5).map((agent) => (
          ...
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${TYPE_DOTS[agent.subagent_type] ?? "bg-indigo-500"}`}
            />
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={`text-[9px] rounded border px-1 ${
                          agent.status === "completed"
                            ? "text-green-600 border-green-500/30"
                            : agent.status.startsWith("running")
                  ...
                  <span
                    className={`h-2 w-2 rounded-full shrink-0 ${TYPE_DOTS[agent.subagent_type] ?? "bg-indigo-500"}`}
                  />
                </button>
              ))}
            </div>

...
function defaultMockResult(cmd: string): unknown {
	// Heuristic: command names like `list_*`, `*_list`, `*s_list`, or
	// `get_*_list` overwhelmingly return arrays in Tauri APIs. Returning [] for
	// those keeps `.find` / `.map` / `.length` from blowing up on the UI side.
	// Everything else defaults to null (safe for `?? fallback` patterns).
	if (
		/^list_/.test(cmd) ||
		/_list$/.test(cmd) ||
		/^get_.*_list$/.test(cmd) ||
		/_rate_limits$/.test(cmd)
	) {
		return [];
	}
	return null;
...
export async function invoke<T = unknown>(
	...
	const result = handler ? handler(args) : defaultMockResult(cmd);
	return result as T;
}

export const convertFileSrc = (
	filePath: string,
	_protocol = "asset",
): string => {

// @ts-nocheck
/* eslint-disable */
// Vendored from @replit/codemirror-vim v6.3.0 — DO NOT EDIT
...
function initVim(CM) {
  ...
  var numberRegex = /[\d]/;
  var wordCharTest = [CM.isWordChar, (ch) => ch && !CM.isWordChar(ch) && !/\s/.test(ch)],
    bigWordCharTest = [(ch) => /\S/.test(ch)];
  var validMarks = ["<", ">"];
  var validRegisters = ["-", '"', ".", ":", "_", "/", "+"];
  var latinCharRegex = /^\w$/;
  var upperCaseChars = /^[A-Z]$/;
  try {
    upperCaseChars = /^[\p{Lu}]$/u;
  } catch (_) {}
  ...
  function getOption(name, cm, cfg) {
    ...
    if (option.callback) {
      ...
      const local = scope !== "global" && cm && cm.state.vim.options[name];
  ...
  defineOption("textwidth", 80, "number", ["tw"], (width, cm) => {
    // Option is local. Do nothing for global.
    if (cm === undefined) {
      return;
    }
    ...
    enterMacroRecordMode(cm, registerName) {
      ...
      if (register) {
        ...
        if (cm.openDialog) {
          ...
          this.onRecordingDone = cm.openDialog(template, () => {}, { bottom: true });
        }
  ...
  */
  ...
  var vimApi = {
    ...
    unmap: (lhs, ctx) => exCommandDispatcher.unmap(lhs, ctx),
    ...
    findKey: (cm_, key, origin) => {
      ...
      var command = vim.insertMode ? handleKeyInsertMode() : handleKeyNonInsertMode();

      if (command === false) {
        return !vim.insertMode && (key.length === 1 || (CM.isMac && /<A-.>/.test(key)))
          ? () => true
          : undefined;
      ...
      while ((match = keyRe.exec(keys))) {
        ...
        if (!result && wasInsert && vim.insertMode) {
          if (key[0] == "<") {
            ...
            else if (Object.hasOwn(vimToCmKeyMap, lowerKey)) {
  ...
  defineOption("langmap", undefined, "string", ["lmap"], (name, cm) => {
    // The 'filetype' option proxies to the CodeMirror 'mode' option.
    if (name === undefined) {
      ...
      if (!bestMatch) {
        if (matches.partial.length) {
          return {
            type: "partial",
            expectLiteralNext:
              matches.partial.length == 1 && matches.partial[0].keys.slice(-11) == "<character>", // langmap literal logic
          };
        }
        return { type: "none" };
      }
  ...
  var motions = {
    ...
    findNext: (cm, _head, motionArgs) => {
      ...
      if (!result) {
        showConfirm(
          cm,
          "No match found " + query + (getOption("pcre") ? " (set nopcre to use Vim regexps)" : ""),
        );
      }
      return result;
    },
     ...
     */
    findAndSelectNextInclusive: (cm, _head, motionArgs, vim, prevInputState) => {
      var state = getSearchState(cm);
      var query = state.getQuery();
    ...
    moveBySentence: (cm, head, motionArgs) => {
      var dir = motionArgs.forward ? 1 : -1;
    ...
    moveTillCharacter: (cm, head, motionArgs) => {
      var repeat = motionArgs.repeat;
    ...
    moveToCharacter: (cm, head, motionArgs) => {
      ...
      return (
        moveToCharacter(cm, repeat, motionArgs.forward, motionArgs.selectedCharacter, head) || head
      );
    },
    ...
    moveToEol: (cm, head, motionArgs, vim) => moveToEol(cm, head, motionArgs, vim, false),
    ...
    moveToStartOfLine: (_cm, head) => new Pos(head.line, 0),
    ...
    moveToStartOfDisplayLine: (cm) => {
      cm.execCommand("goLineLeft");
      return cm.getCursor();
    },
    moveToEndOfDisplayLine: (cm) => {
  ...
  var operators = {
    ...
    indentAuto: (cm, _args, ranges) => {
      cm.execCommand("indentAuto");
      return motions.moveToFirstNonWhiteSpaceCharacter(cm, ranges[0].anchor);
    },
    hardWrap: (cm, operatorArgs, ranges, oldAnchor) => {
    ...
    rot13: (cm, args, ranges, oldAnchor, newHead) => {
      var selections = cm.getSelections();
      var swapped = [];
      for (var j = 0; j < selections.length; j++) {
  ...
  var actions = {
    ...
    continuePaste: (cm, actionArgs, vim, text, register) => {
      ...
      if (actionArgs.matchIndent) {
        ...
        var whitespaceLength = (/** @type {string} */ str) => {
    ...
    insertRegister: (cm, actionArgs, vim) => {
      var registerName = actionArgs.selectedCharacter;
      var register = vimGlobalState.registerController.getRegister(registerName);
    ...
    oneNormalCommand: (cm, actionArgs, vim) => {
    ...
    indent: (cm, actionArgs) => {
      cm.indentLine(cm.getCursor().line, actionArgs.indentRight);
    },
    exitInsertMode: (cm, actionArgs) => {
      exitInsertMode(cm);
    },
  };
        ...
        number == "decimal"
          ...
          : number == "hex"
            ...
            : number == "octal"
              ...
              : null;
      var radix =
        number == "decimal" ? 10 : number == "hex" ? 16 : number == "octal" ? 8 : undefined;
      var numPart = [],
        textPart = [];
      if (number || pattern) {
  ...
  function doReplace(
    ...
    function replace() {
      ...
      if (match) {
        newText = replaceWith.replace(/\$(\d{1,3}|[$&])/g, (_, x) => {
          if (x == "$") return "$";
          if (x == "&") return match[0];
          var x1 = x;
    ...
    if (done) {
      showConfirm(
        cm,
        "No matches for " + query + (getOption("pcre") ? " (set nopcre to use vim regexps)" : ""),
      );
      return;
    }
    if (!confirm) {
  ...
  function addUsedKeys(keys) {
    keys.split(/(<(?:[CSMA]-)*\w+>|.)/i).forEach((part) => {
      if (part) {

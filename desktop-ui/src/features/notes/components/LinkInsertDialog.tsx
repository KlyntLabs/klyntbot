      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            className="w-full px-3 py-2 text-sm rounded-lg bg-surface-base border border-border text-primary placeholder:text-dim outline-none focus:border-brand/40 transition-colors"
          />

          {/* Image preview */}
          {type === "image" && url.trim() && (
            <div className="mt-3 rounded-lg overflow-hidden bg-surface-low flex items-center justify-center min-h-[80px] max-h-[160px]">
              {!previewError ? (
                <img
                  src={url.trim()}
                  alt="Preview"
          ...
          <div className="flex items-center justify-end gap-2 mt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-3 py-1.5 text-xs rounded-lg text-muted hover:text-secondary hover:bg-surface-base transition-colors"
            >
              Cancel
            </button>
            <button

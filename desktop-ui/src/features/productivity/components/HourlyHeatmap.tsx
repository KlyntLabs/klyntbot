  ...
  return (
    <div className="space-y-1 px-1 py-2">
      <div className="text-xs font-medium text-foreground">
        Hourly Productivity
        {peakHour && <span className="text-muted font-normal ml-1">Peak: {peakHour.hour}:00</span>}
      </div>
        ...
        {working.map((h) => {
          ...
          return (
            <div key={h.hour} className="flex items-center gap-1.5">
              <span className="text-[10px] text-muted w-6 text-right tabular-nums">{h.hour}</span>

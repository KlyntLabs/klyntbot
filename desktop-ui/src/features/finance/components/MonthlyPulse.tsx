...
export function MonthlyPulse({ rows }: { rows: PulseRow[] }) {
  return (
    <div className="flex flex-col justify-center h-full">
      <p className="text-2xs text-muted-foreground uppercase tracking-widest mb-4">Monthly Pulse</p>
      {rows.map((row) => (
        <div key={row.label} className="flex items-center gap-3 mb-3.5 last:mb-0">
          <div
            className="size-9 rounded-xl flex items-center justify-center text-base font-light flex-shrink-0"
          ...
          <div className="flex-1">
            ...
            <p className="text-2xs text-dim mt-0.5">{row.hint}</p>

...
export function computeMonthlyPulse(params: {
  ...
  const incPct =
    previousIncome > 0 ? Math.round(((currentIncome - previousIncome) / previousIncome) * 100) : 0;
  const spendPct =
    previousSpending > 0
  ...
  const curSavingsRate =
    currentIncome > 0 ? Math.round(((currentIncome - currentSpending) / currentIncome) * 100) : 0;
  const prevSavingsRate =
    previousIncome > 0
      ? Math.round(((previousIncome - previousSpending) / previousIncome) * 100)
      : 0;
  ...
  const savDir: PulseRow["direction"] =
    curSavingsRate > prevSavingsRate + 2
      ? "up"
      : curSavingsRate < prevSavingsRate - 2
        ? "down"
        : "flat";
  ...
  return [
    {
      ...
      hint:
        incDir === "flat"
          ? "Stable · on track"
          : `${incPct > 0 ? "+" : ""}${incPct}% ${incPct > 0 ? "higher" : "lower"} · ${incPct > 0 ? "on track" : "watch this"}`,
    ...
    },
    {
      ...
      hint:
        spendDir === "flat"
          ? "Stable"
          : `${spendPct > 0 ? "+" : ""}${spendPct}% ${spendPct > 0 ? "higher" : "lower"} · ${spendPct < 0 ? "improving" : "watch this"}`,

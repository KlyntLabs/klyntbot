...
function TodayStats({
	...
	return (
		<div className="tc-today-stats">
			Today: {stats.sessions} session{stats.sessions !== 1 ? "s" : ""} ·{" "}
			{timeStr}
...
function TimerView({
	...
	const progress =
		isActive && remainingSecs != null && totalSecs
			? (totalSecs - remainingSecs) / totalSecs
			: 0;
	...
	return (
		<div className="tc-timer">
			...
			<div
				className="tc-ring-wrap"
				style={{ width: RING_SIZE, height: RING_SIZE }}
			>
				<svg
					viewBox={`0 0 ${RING_SIZE} ${RING_SIZE}`}
					className="tc-ring-svg"
					aria-hidden="true"
				>
					<circle
						cx={CENTER}
				...
				<div className="tc-ring-content">
					...
					<div className="tc-dots">
						{Array.from({ length: dotsCount }, (_, i) => (
							<div
								key={`dot-${i}`}
								className={`tc-dot${i < filledDots ? " is-filled" : ""}`}
							/>
						))}
					</div>
					...
					{showLearningLine && (
						<p className="tc-learning-banner">Learning how you focus best...</p>
					)}
				</div>
			</div>
				...
				<div className="tc-presets">
					{FOCUS_PRESETS.map((preset) => (
						<button
							...
							className={`tc-preset${
								timer.activePreset === preset.label ? " is-active" : ""
							}`}
						>
		...
		<div className="tc-settings-row">
			...
			{editing ? (
				...
				<button type="button" onClick={startEdit} className="tc-settings-value">
					<span className="tc-settings-num">

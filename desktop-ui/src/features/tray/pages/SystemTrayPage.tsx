            ...
            <div className="px-4 py-3" style={{ animation: "nudge-slide-in 0.25s ease-out" }}>
              ...
              <div className="flex items-center gap-1 mt-2 ml-6">
                <button
                  ...
                  title="Helpful"
                  className="h-6 px-2 flex items-center gap-1 rounded-md text-ui-xs text-fg-secondary hover:text-status-success hover:bg-control-hover transition-colors"
                >
                ...
                <button
                  ...
                  title="Dismiss"
                  className="h-6 px-2 flex items-center gap-1 rounded-md text-ui-xs text-fg-secondary hover:text-fg hover:bg-control-hover transition-colors"
                >
                ...
                <button
                  ...
                  className="h-6 px-2 flex items-center gap-1 rounded-md text-ui-xs text-fg-secondary hover:text-status-danger hover:bg-control-hover transition-colors"
                >
            ...
            <>
              ...
              <div className="px-4 py-3 overflow-y-auto max-h-[240px]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-ui-xs text-fg-secondary font-light uppercase tracking-wider">
                    Today
                  </span>
                  {activeCount > 0 && (
                    <span className="glass-badge px-2 py-0.5 text-ui-xs text-fg-secondary font-light">
                      {activeCount}
                    </span>
                  )}
                </div>
                <div className="space-y-0.5">
                  {sortedTasks.map((task) => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className={`flex items-center gap-2.5 py-1.5 px-2 rounded-lg border-l-2 border-y-0 border-r-0 hover:bg-control-hover transition-colors ${taskIndicatorClass(task, done)}`}
                      >
                        <Checkbox
                          checked={done}
                        ...
                        <button
                          ...
                          className={`flex-1 text-ui-sm font-light truncate text-left hover:underline ${
                            done
                        ...
                        {!done && task.priority && (
                          <Badge variant="brand" className="text-ui-xs px-1.5 py-0">
                            {task.priority}
                          </Badge>
                        )}
                        {!done && task.dueDisplay && (
                          <span
                            className={`text-ui-xs font-light flex-shrink-0 ${
                              task.isOverdue ? "text-status-danger" : "text-fg-secondary"
                            }`}
                          >
                            {task.dueDisplay}
                          </span>
                  ...
                  {calendarEvents.map((event) => (
                    <div key={event.id} className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-ui-sm font-light text-fg-secondary truncate">
                          {event.title}
                        </p>
                      </div>
                      <span className="text-ui-xs text-fg-dim font-light flex-shrink-0">

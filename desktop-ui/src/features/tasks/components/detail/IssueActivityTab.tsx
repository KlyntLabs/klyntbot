...
function ActivityEntry({ entry }: { entry: ActivityEntryType }) {
  return (
    <div className="flex gap-3 py-3 border-b border-separator/50 last:border-b-0">
      <ActorAvatar type={entry.actorType} />
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline gap-2">
          <span className="text-sm font-medium text-fg">{entry.actorName}</span>
          <span className="text-sm text-fg-secondary">{entry.action}</span>
          <span className="ml-auto text-ui-sm text-fg-secondary shrink-0">
            {formatActivityTime(entry.createdAt)}
          </span>
        </div>
        {entry.detail && <p className="text-sm text-fg-secondary mt-0.5">{entry.detail}</p>}
      </div>
    </div>
  );
}
...
function ActorAvatar({ type }: { type: ActorType }) {
  ...
  if (type === "system") {
    return (
      <div className="size-7 rounded-full shrink-0 flex items-center justify-center bg-bg-elevated">
        <Monitor className="size-3.5 text-fg-secondary" />
      </div>
    );
  }
  return (
    <div className="size-7 rounded-full shrink-0 flex items-center justify-center bg-control-hover">
      <User className="size-3.5 text-fg" />
    </div>
  );
}

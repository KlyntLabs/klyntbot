      ...
      <div className="flex items-center gap-1 min-w-0">
        {navStack.map((entry, index) => {
          ...
          return (
            ...
            >
              {index > 0 && <span className="text-ui-sm text-fg-secondary flex-shrink-0">›</span>}
              {isLast ? (
                <span className="text-sm font-medium text-fg truncate">{entry.label}</span>
              ) : (
                <button
                  type="button"
                  onClick={() => navigateToStackIndex(index)}
                  className="text-sm text-fg-secondary hover:text-fg transition-colors truncate"
                >
                  {entry.label}
                </button>
              )}
        ...
        {isSearchOpen ? (
          <div className="flex items-center gap-2">
            <div className="relative">
              <Search className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-fg-secondary" />
              <input
                ref={inputRef}
                type="text"
                ...
                className="h-7 w-[200px] rounded-md border border-separator bg-bg pl-8 pr-2 text-sm text-fg placeholder:text-fg-secondary outline-none focus:ring-1 focus:ring-fg-secondary/30"
              />
            </div>

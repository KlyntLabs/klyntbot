    ...
    <ContextMenu>
      ...
      <ContextMenuContent className="w-64">
        <ContextMenuGroup>
          ...
          >
            Copy ID
            <span className="ml-auto text-ui-sm text-fg-secondary">{issue.identifier}</span>
          </ContextMenuItem>
          <ContextMenuItem
            onSelect={() => {
              useTabStore
            ...
            {statuses.map((s) => (
              ...
              >
                ...
                {issue.status.id === s.id && (
                  <span className="ml-auto text-ui-sm text-fg-secondary">Current</span>
                )}
              </ContextMenuItem>
            ))}
          </ContextMenuSubContent>
        ...
        <ContextMenuSub>
          <ContextMenuSubTrigger>
            <issue.priority.icon className="mr-2 h-4 w-4 text-fg-secondary" />
            Priority
          </ContextMenuSubTrigger>
          <ContextMenuSubContent className="w-48">
            {priorities.map((p) => {

        ...
        <li key={s.sessionId}>
          <button
            ...
            className={
              ...
              (selectedId === s.sessionId ? " cm-session-list__item--active" : "")
            }
            onClick={() => onSelect(s.sessionId)}
          >
            <div className="cm-session-list__top">
              <span className={`cm-event-chip cm-event-chip--${providerColor(s.source)}`}>
                {s.source}
              </span>
              <span className="cm-session-list__id">{s.sessionId.slice(0, 8)}…</span>
              <span className="cm-session-list__when">
            ...
            <div className="cm-session-list__stats">
              ...
              {s.errorCount > 0 && (
                <span className="cm-session-list__errors"> · {s.errorCount} errors</span>
              )}
            </div>
          </button>
        </li>

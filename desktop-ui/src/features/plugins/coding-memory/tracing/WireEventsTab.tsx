...
import type { Scope, TraceEvent } from "./types";
...
export function WireEventsTab(props: Props) {
  ...
  const selectedToolEvent = useMemo(() => {
    ...
    return events.find(
      (e) => (e.payload as any)?.id === selectedToolId && e.rawKind === "ToolCall",
    );
      ...
      <div className="tracing-detail-pane__left">
        ...
        <Virtuoso
          ...
          itemContent={(index, event) => {
            ...
            const deltaMs = prev
              ? new Date(event.occurredAt).getTime() - new Date(prev.occurredAt).getTime()
              : undefined;
...
function renderCard(event: TraceEvent, ctx: CardCtx) {
  switch (event.category) {
    case "turnBegin":
      return <TurnBeginCard event={event} expanded={ctx.expanded} onToggle={ctx.onToggle} />;
    case "turnEnd":
      return <TurnEndCard event={event} />;
    case "stepBegin":
      return <StepBeginCard event={event} deltaMs={ctx.deltaMs} />;
    case "stepInterrupted":
      return <StepInterruptedCard event={event} />;
    case "thinking":
      return <ThinkingCard event={event} />;
    case "assistantText":
      return <AssistantTextCard event={event} />;
    case "toolCall":
      return (
        <ToolCallCard event={event} onSelect={ctx.onSelectTool!} selected={ctx.selectedTool} />
      );
    case "toolCallStream":
      return <ToolCallStreamCard event={event} />;
    case "toolResult":
      return <ToolResultCard event={event} />;
    case "statusUpdate":
      return <StatusUpdateCard event={event} />;
    case "subagent":
      return <SubagentEventCard event={event} onOpenSubagent={ctx.onOpenSubagent} />;

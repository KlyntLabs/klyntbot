...
export function AgentsTab({ providerName: _providerName, scope: _scope, providerId, sessionId }: Props) {
  const [agents, setAgents] = useState<SubagentSummary[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!providerId || !sessionId) { setAgents([]); return; }
    let cancelled = false;
    setLoading(true);
    ...
    <div className="tracing-agents">
      ...
      {agents.map((a) => (
        <div key={a.agentId} className="tracing-agents__card">
          ...
          <div className="tracing-agents__stats">{a.eventCount} events · {a.status}</div>
        </div>
      ))}
    </div>
  );

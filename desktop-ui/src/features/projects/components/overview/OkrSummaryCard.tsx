    ...
    <div className="island rounded-xl p-5">
        ...
        {top3.map((obj) => (
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-ui-sm text-fg truncate max-w-[70%]">{obj.title}</span>
              <span className="text-ui-xs text-fg-secondary font-medium">
      ...
      {objectives.length > 3 && (
        <button
          type="button"
          onClick={() => navigate(`/project/${project?.id ?? ""}/okr`)}
          className="text-ui-xs text-brand mt-2 hover:underline"
        >

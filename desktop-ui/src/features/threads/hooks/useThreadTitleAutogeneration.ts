...
function cleanPromptText(text: string) {
  ...
  return normalized.length > MAX_PROMPT_CHARS ? normalized.slice(0, MAX_PROMPT_CHARS) : normalized;
}
...
export function useThreadTitleAutogeneration({
  ...
  const onUserMessageCreated = useCallback(
    ...
    [enabled, getCustomName, itemsByThreadRef, threadsByWorkspaceRef, renameThread, onDebug],
  );

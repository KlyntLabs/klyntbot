...
export interface CurrentWindow {
  ...
  onFocusChanged(handler: (e: { payload: boolean }) => void): Promise<() => void>;
...
export function currentWindowLabel(): string {
  ...
  if (typeof window.location !== "undefined") {
    ...
    if (window.location.hash === "#/distraction-overlay") return "distraction-overlay";
  }
  return "main";
}
...
export function getWindowByLabel(label: string): CurrentWindow {
  return {
    ...
    startDragging: () => invoke<void>("plugin:window|start_dragging", { label }),
    ...
    onFocusChanged: async (handler) => {
      const offFocus = await listen<null>("tauri://focus", () => handler({ payload: true }));
      const offBlur = await listen<null>("tauri://blur", () => handler({ payload: false }));
      return () => {
        offFocus();
        offBlur();
      };

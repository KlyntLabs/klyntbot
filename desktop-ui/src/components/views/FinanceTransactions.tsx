...
import { ArrowUpRight, ArrowDownRight, ArrowLeftRight, Search, Plus } from 'lucide-react';
import { useQuery } from '../../hooks/useQuery';
import { useEvent } from '../../hooks/useEvent';
import { cn } from '../../lib/utils';
import { fmtMoney, toVnd, fmtCompact, COLORS } from '../../lib/finance';
...
export function FinanceTransactions() {
  ...
  const totalIncome = useMemo(() => filtered.filter(t => t.txType === 'income').reduce((s, t) => s + toVnd(t.amount, t.currency, rates), 0), [filtered, rates]);
  const totalExpense = useMemo(() => filtered.filter(t => t.txType === 'expense').reduce((s, t) => s + toVnd(t.amount, t.currency, rates), 0), [filtered, rates]);
    ...
    <FinanceLayout onRefresh={refetchAll}>
      <div className="grid grid-cols-12 gap-3 auto-rows-min">
        ...
        <div className="col-span-12 grid grid-cols-4 gap-3">
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Total Transactions</p>
          ...
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Expenses</p>
          ...
          <Card className="p-4">
            ...
            <p className={cn('text-[20px] font-light', totalIncome - totalExpense >= 0 ? 'text-success' : 'text-destructive')}>
              {totalIncome - totalExpense >= 0 ? '+' : ''}{fmtCompact(totalIncome - totalExpense)}đ
            </p>
          </Card>
        </div>
        ...
        <div className="col-span-12 flex items-center gap-3">
            ...
            {filters.map(f => (
              <button
                ...
                className={cn(
                  ...
                  filter === f.key ? 'bg-surface-highest text-brand' : 'text-muted hover:text-secondary hover:bg-surface-base',
                )}
              >
                {f.label}
              </button>
          ...
          <div className="relative">
            <Search className="w-3 h-3 text-dim absolute left-2 top-1/2 -translate-y-1/2" strokeWidth={1.5} />
            <input
              type="text"
              value={searchQ}
          ...
          <Card className="overflow-hidden">
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(tx => {
                ...
                return (
                  <div key={tx.id} className="grid grid-cols-[70px_24px_1fr_80px_100px_120px] gap-2 items-center px-4 py-2.5 hover:bg-surface-base transition-colors border-b border-border-subtle last:border-b-0">
                    <span className="text-[10px] text-dim font-light">{tx.txDate}</span>
                    <TxI className={cn('w-3.5 h-3.5', col)} strokeWidth={1.5} />
                    <div className="min-w-0">
                      <p className="text-[12px] font-light text-secondary truncate">{tx.counterparty ?? tx.notes ?? tx.txType}</p>
                      {tx.subcategory && <p className="text-[9px] text-dim font-light">{tx.subcategory}</p>}
                    </div>
                    <span className="px-1.5 py-0.5 text-[9px] font-light rounded bg-surface-base text-dim truncate">{tx.category ?? '—'}</span>
                    <span className="text-[10px] text-dim font-light truncate">{acct?.name ?? '—'}</span>
                    <span className={cn('text-[12px] font-light text-right', col)}>{pre}{fmtMoney(tx.amount, tx.currency)}</span>
                  </div>
                );
              })
            )}
        ...
        <div className="col-span-3">
          ...
          <Card className="p-4">
            {catSegs.length > 0 ? (
              <>
                <Donut segments={catSegs} label="Spending" value={fmtCompact(totalExpense) + 'đ'} size={140} />
                <div className="mt-3 pt-2.5 border-t border-border-subtle space-y-1.5">
                  {catSegs.map((seg, i) => (
                    <div key={i} className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] text-secondary font-light">{fmtCompact(seg.value)}đ</span>
                    </div>
                  ))}
                </div>
              </>

...
import { fmtMoney, fmtCompact, pct, GOAL_ICONS, COLORS } from '../../lib/finance';
...
export function FinanceGoals() {
  ...
  const totalTarget = useMemo(() => activeGoals.reduce((s, g) => s + g.targetAmount, 0), [activeGoals]);
  const totalSaved = useMemo(() => activeGoals.reduce((s, g) => s + g.currentAmount, 0), [activeGoals]);
  const monthlyTotal = useMemo(() => activeGoals.reduce((s, g) => s + (g.monthlyContribution ?? 0), 0), [activeGoals]);

  const goalSegs = useMemo(() =>
    activeGoals.map((g, i) => ({ name: g.name, value: g.currentAmount, color: COLORS[i % COLORS.length] })),
    [activeGoals],
  );
    ...
    <FinanceLayout onRefresh={refetch}>
      <div className="grid grid-cols-12 gap-3 auto-rows-min">
        ...
        <div className="col-span-12 grid grid-cols-4 gap-3">
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Active Goals</p>
            <p className="text-[20px] font-light text-primary">{activeGoals.length}</p>
          </Card>
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Total Saved</p>
            <p className="text-[20px] font-light text-success">{fmtCompact(totalSaved)}đ</p>
          </Card>
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Total Target</p>
            <p className="text-[20px] font-light text-primary">{fmtCompact(totalTarget)}đ</p>
          </Card>
          <Card className="p-4">
            <p className="text-[10px] text-dim font-light uppercase tracking-wider mb-1">Monthly Contributions</p>
        ...
        <div className="col-span-8">
          ...
          <div className="space-y-3">
            {activeGoals.map((g, i) => {
              ...
              const monthsLeft = g.monthlyContribution && g.monthlyContribution > 0
                ...
                <Card key={g.id} className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-right">
                      <p className="text-[14px] font-light text-primary">{fmtMoney(g.currentAmount, g.currency)} <span className="text-dim text-[10px]">/ {fmtMoney(g.targetAmount, g.currency)}</span></p>
                      <p className="text-[11px] font-light text-brand">{p}%</p>
                    </div>
                  </div>
                  <Progress value={p} />
                  <div className="flex items-center gap-4 mt-2">
                    {g.deadline && <span className="text-[10px] text-dim font-light">Deadline: {g.deadline}</span>}
                    {g.monthlyContribution && <span className="text-[10px] text-dim font-light">Contributing: {fmtMoney(g.monthlyContribution, g.currency)}/mo</span>}
                    {monthsLeft != null && (
                      <span className="text-[10px] text-muted font-light">
                        ~{monthsLeft} months to go
                      </span>
                    )}
                    <span className="text-[10px] text-dim font-light">{fmtMoney(remaining, g.currency)} remaining</span>
                  </div>
                </Card>
              );
            })}
        ...
        <div className="col-span-4 space-y-3">
          <div>
            ...
            <Card className="p-4 flex items-center justify-center">
              <Donut segments={goalSegs} label="Saved" value={fmtCompact(totalSaved) + 'đ'} size={150} />
            </Card>
          </div>
          <div>
            ...
            <Card className="p-4">
              <div className="mb-3">
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-[10px] text-brand font-light">{pct(totalSaved, totalTarget)}%</span>
                </div>

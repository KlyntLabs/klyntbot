...
interface ScheduleFields {
  ...
  weekdays: boolean[];
  time: string;
  /** ISO date (YYYY-MM-DD) for the calendar */
  date: string;
  dateTime: string;
}
...
function SchedulePanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── Time picker (always visible for once + cron) ── */}
...
function InlineScheduleCell({
  ...
  const handleOpen = (e: React.MouseEvent) => {
    ...
    if (triggerRef.current) {
      ...
      // Vertical: prefer below, flip above, clamp
...
// ── Schedule Builder (for Create Form — wraps SchedulePanel) ────────────

...
impl ApprovalHistorySource {
    ...
    pub fn observe_request(&self, request_id: &str, tool: &str, args_hash: &str, layer: &str, repo_id: &str) {
        self.pending.insert(request_id.to_string(), PendingReq {
            tool: tool.to_string(), args_hash: args_hash.to_string(),
            layer: layer.to_string(), repo_id: repo_id.to_string(),
        });
    }
    ...
    pub async fn observe_resolution(&self, request_id: &str, decision: &str, decided_by: &str) {
        if let Some((_, pending)) = self.pending.remove(request_id) {
            let _ = self.repo.record(HistoryEntry {
                ...
                repo_id: pending.repo_id, decision: decision.to_string(),
                decided_by: decided_by.to_string(), layer: pending.layer,
            }).await;
        }
    }
    ...
    pub async fn observe(&self, ev: &ToolEvent, repo_id: &str) {
        match ev {
            ...
            ToolEvent::ApprovalResolved { request_id, decision, decided_by, .. } => {
                self.observe_resolution(request_id, decision, decided_by).await;
            }
            _ => {}
        }
    }

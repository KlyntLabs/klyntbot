...
use klyntbot_providers::{ChatParams, DynProvider, Message, tool_calls_to_messages};
...
impl AgentLoop {
    ...
    pub async fn new_with_cron(
        ...
        provider: DynProvider,
        config: Config,
        cron_service: Option<Arc<klyntbot_cron::CronService>>,
    ) -> Result<Self> {
        ...
        context_builder.init().await.map_err(|e| {
            klyntbot_core::KlyntbotError::Config(klyntbot_core::ConfigError::Invalid(format!(

...
async fn enter_sets_approval_mode_plan_and_emits_event() {
    ...
    let key = repos
        .sessions
        .upsert_session("u1", &serde_json::json!({}))
        .await
        .unwrap()
        .key;
    let bus = Arc::new(DomainEventBus::new(64));
    let (tx, mut rx) = mpsc::channel(32);
    run_enter_for_test(&repos, &key, Some(tx.clone()), bus.clone())
        .await
        .unwrap();
    ...
    while let Some(e) = rx.recv().await {
        if let ToolEvent::PlanModeChanged {
            in_plan_mode: true, ..
        } = e
        {
            saw = true;
...
async fn exit_sets_approval_mode_default_and_emits_event() {
    ...
    repos
        .sessions
        .update_approval_mode(&key, "plan")
        .await
        .unwrap();
    let bus = Arc::new(DomainEventBus::new(64));
    let (tx, mut rx) = mpsc::channel(32);
    run_exit_for_test(&repos, &key, Some(tx.clone()), bus.clone())
        .await
        .unwrap();
    let row = repos.sessions.get_session(&key).await.unwrap();

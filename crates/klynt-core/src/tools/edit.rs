...
pub async fn run_for_test(
    ...
    if let Err(reason) = fire_pre_tool_use(hook_engine.as_ref(), session_id.clone(), "edit", args_json, None).await {
    ...
    let result: Result<String> = (async {
        ...
        emit_file_edit(&event_tx, &bus, FileEditEvent {
            op: "edit", path: &path_str, bytes: final_content.len() as u64, diff_full: diff,
        }).await;
        Ok(format!("edited {} ({} bytes)", path_str, final_content.len()))
    }).await;
    fire_post_tool_use(hook_engine.as_ref(), session_id, "edit", result.is_ok(), start.elapsed().as_millis() as u64).await;
    result
}

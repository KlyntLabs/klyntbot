...
pub fn hybrid_merge(
    keyword_results: &[Action],
    semantic_results: &[(String, f64)],
    rrf_k: u32,
) -> Vec<(Action, f64, &'static str)> {
    let items_by_id: HashMap<String, Action> = keyword_results
        .iter()
...
mod tests {
    ...
    fn make_action(id: &str, title: &str) -> Action {
    ...
    fn test_hybrid_merge_keyword_only() {
        let actions = vec![make_action("a1b2c3d4", "fix auth bug")];
        let semantic: Vec<(String, f64)> = vec![];
        let merged = hybrid_merge(&actions, &semantic, 60);
    ...
    fn test_hybrid_merge_both_sources() {
        let action = make_action("a1b2c3d4", "auth bug");
        let actions = vec![action.clone()];
        let semantic = vec![(action.id.clone(), 0.85)];
        let merged = hybrid_merge(&actions, &semantic, 60);
        assert_eq!(merged.len(), 1);
    ...
    fn test_rrf_score_ordering() {
        let t1 = make_action("aaaa0001", "task one");
        let t2 = make_action("aaaa0002", "task two");
        let actions = vec![t1.clone(), t2.clone()];

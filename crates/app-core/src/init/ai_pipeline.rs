...
fn try_into_task_event(e: &DomainEvent) -> Option<feature_tasks::events::TaskEvent> {
    ...
    match e {
        DomainEvent::TaskCreated { task_id, project, estimate_mins, .. } =>
        ...
        DomainEvent::TaskCompleted { task_id, deviation_pct, .. } =>
            Some(TaskEvent::Completed {
                task_id: task_id.clone(),
                title: String::new(), // title not in DomainEvent::TaskCompleted
                deviation_pct: *deviation_pct,
            }),
        DomainEvent::TaskFocusChanged { task_id, focus_deadline, .. } =>
        ...
        DomainEvent::EstimationRecorded { task_id, estimated_mins, actual_mins, .. } =>

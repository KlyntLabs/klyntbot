...
impl AppCorePlugin for LauncherPlugin {
    ...
    async fn init(&self, ctx: &mut PluginContext) -> common::Result<()> {
        ...
        let result =
            self::init::init_launcher(&config, &ctx.deps.storage_pool, &ctx.deps.shutdown_token)
                .await;
...
mod init {
    ...
    pub async fn init_launcher(
        ...
        if let Err(e) =
            StoragePool::run_feature_migrations(&pool, &feature_launcher::launcher_migrations())
                .await
        {

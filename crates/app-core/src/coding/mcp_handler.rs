...
impl AppCore {
    ...
    pub async fn coding_mcp_status(&self, workspace_id: Option<&str>) -> Result<McpStatusResult> {
        ...
        for server_cfg in &mcp_config.servers {
            let state = if server_cfg.enabled {
                "configured"
            } else {
                "disabled"
            };
        ...
        tracing::info!(server_count = servers.len(), "MCP status queried");

...
impl CalendarFetcher for AppCalendarFetcher {
    async fn upcoming_events(&self, lookback_days: u32, lookahead_days: u32) -> Vec<LauncherCalendarEvent> {
        ...
        let from = now.saturating_sub(jiff::SignedDuration::from_secs(lookback_days as i64 * day_secs)).unwrap_or(now);
        let to = now.saturating_add(jiff::SignedDuration::from_secs(lookahead_days as i64 * day_secs)).unwrap_or(now);

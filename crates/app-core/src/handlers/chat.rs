//! Chat handlers — wired to AgentLoop with streaming events.
...
type ActiveStreams = dashmap::DashMap<String, CancellationToken>;
...
//
// These take explicit references so they can be called from either the
// app-core `AppCore` or the desktop's own `AppCore` (which has the same
// fields but is a different type during the migration period).

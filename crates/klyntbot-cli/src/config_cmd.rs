...
pub async fn handle_config(cmd: ConfigCommands) -> Result<()> {
    match cmd {
        ...
        ConfigCommands::Set { key, value } => {
            let config_path = klyntbot_config::config_path()?;
            ...
            if set_config_value(&mut json, &key, parsed_value)? {
                ...
                let _: klyntbot_config::Config = serde_json::from_value(json.clone())?;
        ...
        ConfigCommands::Reset { force } => {
            ...
            let default_config = klyntbot_config::Config::default();
            klyntbot_config::save(&default_config)?;

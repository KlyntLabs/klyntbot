...
use klyntbot_channels::ChannelManager;
...
pub async fn handle_serve(port: u16) -> Result<()> {
    ...
    info!("Configuration loaded from: {:?}", klyntbot_config::config_path());

    // Initialize LLM provider
    let provider = klyntbot_providers::create_provider(&config)?;
    ...
    // Wire heartbeat to publish messages through the bus
    {
        ...
        heartbeat_service.set_callback(Arc::new(move |prompt: &str| {
            ...
            rt.block_on(async {
                let msg =
                    klyntbot_bus::InboundMessage::new("system", "heartbeat", "heartbeat", prompt);

...
pub(crate) async fn dispatch_dev(
    ...
    Some(match cmd {
        "coding_skills_list" => {
            dev::val(core.coding_skills_list().await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_info" => {
            let name = try_field!(dev::get_str(body, "name"));
            dev::val(core.coding_skills_info(&name).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_install" => {
            let source = try_field!(dev::get_str(body, "source"));
            dev::val(core.coding_skills_install(source).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_update" => {
            let name = try_field!(dev::get_str(body, "name"));
            dev::val(core.coding_skills_update(&name).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_uninstall" => {
            let name = try_field!(dev::get_str(body, "name"));
            dev::val(core.coding_skills_uninstall(&name).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_toggle" => {
            let name = try_field!(dev::get_str(body, "name"));
            let enabled: bool = try_field!(dev::require(body, "enabled"));
            dev::val(core.coding_skills_toggle(&name, enabled).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_validate" => {
            let name = try_field!(dev::get_str(body, "name"));
            dev::val(core.coding_skills_validate(&name).await.map_err(desktop_shared::errors::ApiError::from))
        }
        "coding_skills_reload" => {
            dev::val(core.coding_skills_reload().await.map_err(desktop_shared::errors::ApiError::from))
        }
        _ => return None,
    })
}

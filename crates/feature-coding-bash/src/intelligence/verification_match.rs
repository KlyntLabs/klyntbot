...
mod tests {
    ...
    fn matches_each_verb() {
        assert_eq!(
            classify("Run integration tests"),
            Some(VerificationVerb::Run)
        );
        assert_eq!(
            classify("Test the supervisor"),
            Some(VerificationVerb::Test)
        );
        assert_eq!(classify("Check migrations"), Some(VerificationVerb::Check));
        assert_eq!(
            classify("Verify migration safety"),
            Some(VerificationVerb::Verify)
        );
        assert_eq!(
            classify("Build release binary"),
            Some(VerificationVerb::Build)
        );
    }
    ...
    fn handles_conjugations() {
        ...
        assert_eq!(
            classify("Verifying constraints"),
            Some(VerificationVerb::Verify)
        );

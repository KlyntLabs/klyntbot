//! Update, complete, enrich, focus, unfocus, move, attach, detach, and log_time action handlers.

use crate::params::ParamExtractor;
use crate::todo_types::{AttachmentType, TodoPatch, TodoStatus};
use chrono::Utc;
...
impl TodoTool {
    pub(crate) async fn handle_update(&self, p: &ParamExtractor<'_>) -> Result<String> {
        ...
        match self.repo.update(&storage_patch).await {
            Ok(row) => {
                let todo = crate::todo_types::Todo::from(row);

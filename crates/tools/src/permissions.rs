...
mod tests {
    ...
    fn test_permission_ordering() {
        assert!(PermissionLevel::Admin > PermissionLevel::Elevated);
        assert!(PermissionLevel::Elevated > PermissionLevel::Standard);
        assert!(PermissionLevel::Standard > PermissionLevel::ReadOnly);
    }

    #[test]
    fn test_permission_granted() {
        let mut perms = ToolPermissions::new(PermissionLevel::Standard);
        perms.set_channel_level("cli", PermissionLevel::Admin);

        assert!(perms.is_allowed("cli", PermissionLevel::Admin));
        assert!(perms.is_allowed("cli", PermissionLevel::Elevated));
        assert!(perms.is_allowed("cli", PermissionLevel::Standard));
        assert!(perms.is_allowed("cli", PermissionLevel::ReadOnly));
    }

    #[test]
    fn test_permission_denied() {
        let mut perms = ToolPermissions::new(PermissionLevel::ReadOnly);
        perms.set_channel_level("telegram", PermissionLevel::Standard);

        assert!(!perms.is_allowed("telegram", PermissionLevel::Elevated));
        assert!(!perms.is_allowed("telegram", PermissionLevel::Admin));
    }

    #[test]
    fn test_default_level_for_unknown_channel() {
        let perms = ToolPermissions::new(PermissionLevel::Standard);

        assert!(perms.is_allowed("unknown_channel", PermissionLevel::Standard));
        assert!(perms.is_allowed("unknown_channel", PermissionLevel::ReadOnly));
        assert!(!perms.is_allowed("unknown_channel", PermissionLevel::Elevated));
    }

    #[test]
    fn test_display() {
        assert_eq!(PermissionLevel::ReadOnly.to_string(), "ReadOnly");
        assert_eq!(PermissionLevel::Standard.to_string(), "Standard");
        assert_eq!(PermissionLevel::Elevated.to_string(), "Elevated");

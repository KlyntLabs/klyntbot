    ...
    fn from(e: LearningEvent) -> Self {
        match e {
            LearningEvent::AtomExtracted {
                ...
                note_id,
                text,
            } => DomainEvent::Learning(BusLearningEvent::KnowledgeAtomExtracted {
                atom_id,
                note_id,
                text,
            }),
            ...
            LearningEvent::AtomReinforced { atom_id } => {
                DomainEvent::Learning(BusLearningEvent::AtomReinforced {
                    atom_id,
            ...
            LearningEvent::FlashcardScheduled {
                ...
                atom_id,
                due_at,
            } => DomainEvent::Learning(BusLearningEvent::FlashcardScheduled {
                flashcard_id: card_id,
                atom_id,
                due_at,
            }),
            LearningEvent::RetentionDecayed { atom_id, retention } => {
                DomainEvent::Learning(BusLearningEvent::AtomRetentionDecayed { atom_id, retention })
            }
            LearningEvent::SemanticFactLinked {
                atom_id,
                fact_id,
                similarity,
            } => DomainEvent::Learning(BusLearningEvent::AtomSemanticFactLinked {
                atom_id,
                fact_id,
                similarity,
            }),
...
pub fn try_from_domain_event(e: &DomainEvent) -> Option<LearningEvent> {
    match e {
        ...
        DomainEvent::Learning(BusLearningEvent::KnowledgeAtomCreated { atom_id, .. }) => {
            Some(LearningEvent::AtomCreated {
                atom_id: atom_id.clone(),
            })
        }
        DomainEvent::Learning(BusLearningEvent::KnowledgeAtomAccepted { atom_id, .. }) => {
            Some(LearningEvent::AtomAccepted {
                atom_id: atom_id.clone(),
            })
        }
        DomainEvent::Learning(BusLearningEvent::KnowledgeAtomArchived { atom_id, .. }) => {
        ...
        DomainEvent::Learning(BusLearningEvent::AtomReinforced { atom_id, .. }) => {

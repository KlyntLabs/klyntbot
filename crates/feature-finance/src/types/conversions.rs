...
impl From<&FinanceBudget> for FinanceBudgetRow {
    fn from(budget: &FinanceBudget) -> Self {
        Self {
            ...
            start_date: budget.start_date.into(),
            end_date: budget.end_date.map(|d| d.into()),
            is_active: budget.is_active,
            alert_threshold: budget.alert_threshold,
            created_at: budget.created_at.into(),
            updated_at: budget.updated_at.into(),
            base_amount: 0,
            base_currency: "USD".to_string(),
            exchange_rate: 1.0,
        }
...
impl From<&FinancePortfolio> for FinancePortfolioRow {
    fn from(portfolio: &FinancePortfolio) -> Self {
        Self {
            ...
            created_at: portfolio.created_at.into(),
            updated_at: portfolio.updated_at.into(),
        }
    }
}
...
impl From<FinanceInvestmentRow> for FinanceInvestment {
    fn from(row: FinanceInvestmentRow) -> Self {
        Self {
            ...
            purchase_date: row.purchase_date.map(|d| *d),
...
impl From<&FinanceLiability> for FinanceLiabilityRow {
    fn from(liability: &FinanceLiability) -> Self {
        Self {
            ...
            due_date: liability.due_date.map(|d| d.into()),
            notes: liability.notes.clone(),
            created_at: liability.created_at.into(),
            updated_at: liability.updated_at.into(),
            base_principal: 0,
            base_remaining: 0,
            base_currency: "USD".to_string(),
            exchange_rate: 1.0,

...
use coding_memory::scope::Sensitivity;

fn fact(id: &str, sensitivity: Sensitivity) -> SerializableSemanticFact {
...
fn drops_high_and_excluded_keeps_normal() {
    let input = vec![
        fact("a", Sensitivity::Normal),
        fact("b", Sensitivity::High),
        fact("c", Sensitivity::Excluded),
        fact("d", Sensitivity::Normal),
    ];

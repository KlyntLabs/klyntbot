...
impl DynamicInjector for PlanModeInjector {
    ...
    fn collect(&self, ctx: &dyn InjectorContext) -> Vec<ContextUpdate> {
        ...
        let Some(lock) = self.policies.get(ctx.thread_id()) else {
            return Vec::new();
        };
        let policy = lock.read();
        let slug = match policy.plan_file_slug() {
            Some(s) => s.to_string(),
            None => return Vec::new(),
        };
        let path = match policy.plan_file_path() {
...
mod tests {
    ...
    fn returns_empty_when_off() {
        ...
        let out = injector.collect(&FakeCtx {
            active: false,
            thread: "t1".into(),
        });
        assert!(out.is_empty());

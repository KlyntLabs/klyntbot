...
fn parse_literal_absolute_path(raw: &str) -> Result<PathBuf> {
...
fn parse_network_rule_decision(raw: &str) -> Result<Decision> {
    match raw {
        "deny" => Ok(Decision::Forbid),
        other => Decision::from_starlark_str(other)
            .ok_or_else(|| Error::InvalidDecision(other.to_string())),
    }
}
...
fn policy_builtins(builder: &mut GlobalsBuilder) {
    fn prefix_rule<'v>(
        ...
        let decision = match decision {
            Some(raw) => Decision::from_starlark_str(raw)
                .ok_or_else(|| anyhow::anyhow!("invalid decision '{raw}'; expected one of: allow, ask, prompt, forbid, forbidden"))?,
            None => Decision::Allow,
        };
...
pub fn parse_to_policy(source: &str, path: &std::path::Path) -> Result<crate::policy::Policy> {
    ...
    parser.parse(path.to_string_lossy().as_ref(), source)?;
    Ok(parser.build())
}

...
- **MCP tool exposure:** Membership is declared on the tool via `Tool::exposure_policy()` / derive `mcp_exposure = "default"|"opt_in"|"forbidden"` (defaults to `Forbidden` — ADR-0102). Empty `config.mcp.server.exposed_tools` is filled from the live `ToolRegistry`'s `McpExposure::Default` tools by `validate_mcp_exposure` / `apply_mcp_exposure_cutover` — not from `default_exposed_tools()`, AiFeatureRegistry, or the retired allowlist. Historical Default names are locked in `HISTORICAL_MCP_DEFAULT_TOOLS` for annotation/migration tests only. Tool names must match the `ToolRegistry` key exactly (e.g. `tasks` not `task`). Rebuild `desktop` — the MCP server ships inside it. Debug with `klyntbot mcp tools --list`.
...
- Domain docs (glossary / ADRs): `docs/agents/domain.md`
